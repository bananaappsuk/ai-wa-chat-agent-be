        ...
        try:
            result = await process_inbound_message(
                InboundMessage(
                    ...
                    message_type=msg.kind or msg.message_type or "text",
                    skip_provider_outbound=True,
                    skip_ai_jobs=bool(msg.media_id) and not (msg.text or "").strip(),
                    business_display_phone=msg.display_phone_number,
                    media_id=msg.media_id,
                    media_filename=msg.filename,
                    media_mime_type=msg.mime_type,
                    media_kind=msg.kind or msg.message_type,
                ),
                db=db,
            )
        except Exception:
